...
def saveTablesFromPage(filePath, tableOutputFolder, pageNum):
    ...
    verbal1image, verbal2image, nonverbalImage = splitThreeTables(
        filePath, linePos
    )

    verbal1Path = os.path.join(
        tableOutputFolder, f"verbal1_page_{pageNum}.png"
    )
    verbal2Path = os.path.join(
        tableOutputFolder, f"verbal2_page_{pageNum}.png"
    )
    nonverbalPath = os.path.join(
        tableOutputFolder, f"nonverbal_page_{pageNum}.png"
    )
...
def splitImage(filePath):
    ...
    verbalArray = image[:, 0:splitCol]
    nonVerbalArray = image[:, splitCol:]
...
def getVerticalLinesPositions(filePath):
    ...
    lines, _ = cv2.findContours(
        imgLinesOnly, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
    )
